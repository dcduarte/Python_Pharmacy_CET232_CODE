...
def open_win():  # OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    flag = 'apt'
    apt = Tk()
    apt.title("Pharmacy Management")
    Label(apt, text="PHARMACY").grid(row=0, column=0, columnspan=3)
    ...
    Button(apt, text='New Customer.', width=25, command=val_cus).grid(row=4, column=0)
...
def refresh():
    ...
    for i in cur:
        ...
        lb2.insert(cx, ' ' + str(i[7]) + '        ' + str(i[3]) + '             £ ' + str(i[4]))
    c.commit()
...
def make_bill():  # makes bill
    ...
    if t == 1:
        ...
        m += "-----------------------------------------------\n"
        m += 'Total' + (' ' * 25) + (' ' * (12 - len(str(ntotal)))) + '£ ' + str(ntotal) + '\n'
        det[3] = str(ntotal)
    else:
        m += 'Total' + (' ' * 25) + (' ' * (12 - len(str(total)))) + '£ ' + str(total) + '\n'
        det[3] = str(total)

    m += "-----------------------------------------------\n\n"
    m += "Signature:_____________________________________\n"
    m += "===============================================\n"
    print(m)
    p = time.localtime()
...
def show_rev():  # opens revenue window-----------------------------------------------------------------------TOTAL REVENUE
    ...
    for i in cur:
        if i[4] == today:
            total += float(i[3])
    print(total)
    Label(rev, width=22, text='Total revenue: £ ' + str(total), bg='black', fg='white').grid(row=1, column=0)
    cx = 0
    ...
    for i in cur:
        if i[4] == today:
            cx += 1
            lb1.insert(cx, 'Bill No.: ' + str(i[5]) + '    : £ ' + str(i[3]))
    c.commit()
...
def search_med():
    ...
    for i in cur:
        if i[5] == sym.get():
            y.append(
                str(i[0]) + '. ' + str(i[1]) + '  £ ' + str(i[4]) + '    Rack : ' + str(i[7]) + '    Mfg : ' + str(
                    i[8]))
            x = x + 1
    top = Tk()
...
def again():  # for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    root = Tk()
    root.title('PHARMACY')
    Label(root, text='MEDPLUS PHARMACY').grid(row=0, column=0, columnspan=5)
    Label(root, text="UNIVERSITY OF SUNDERLAND").grid(row=1, column=0, columnspan=5)
...
def open_cus():  # OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="MEDPLUS PHARMACY").grid(row=0, column=0)
